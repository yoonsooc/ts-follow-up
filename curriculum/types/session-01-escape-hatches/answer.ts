...
function loadConfig(): Config {
  const defaultConfig: Config = { theme: "light", fontSize: 14 };
  ...
  try {
    ...
    if (typeof parsed === 'object' && parsed !== null) {
      const { theme, fontSize } = parsed as { theme?: unknown; fontSize?: unknown };
      if ((theme === "dark" || theme === "light") && typeof fontSize === "number") {
        return { theme, fontSize };
      }
    }
  }
  catch (err) {
    // JSON parsing failed, return default config
